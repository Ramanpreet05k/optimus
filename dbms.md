...
(This requires a Self Join, assuming Employees has a manager_id column)
...
How can we get the system date in MySQL?
SELECT CURDATE();  -- Returns 'YYYY-MM-DD'
SELECT NOW();      -- Returns 'YYYY-MM-DD HH:MM:SS'

SQL vs. NoSQL
SQL (Relational): Data is stored in structured tables with defined schemas. Uses primary/foreign keys to link data. Scales vertically (adding more power to a single server). Best for complex queries and transactional consistency (ACID). Examples: MySQL, PostgreSQL, SQL Server.
NoSQL (Non-Relational): Data is stored in flexible formats like JSON documents, key-value pairs, or graphs. Schemas are dynamic. Scales horizontally (adding more servers). Best for massive, unstructured data and rapid development. Examples: MongoDB, Redis, Cassandra.

Write SQL queries to print alternate tuples (rows)
Using a window function (ROW_NUMBER) combined with a modulo operator:
-- Print Odd Rows
SELECT * FROM (
...
WHERE filters individual rows before any grouping or aggregation takes place. It cannot be used with aggregate functions (like SUM or COUNT).
...
PRIMARY KEY vs. UNIQUE Constraint: Both ensure data is not duplicated. However, a PRIMARY KEY does not allow NULL values (and you can only have one per table). A UNIQUE constraint allows NULL values (and you can have multiple per table).
...
7. Normalization
Normalization is the process of organizing data to minimize redundancy (duplicate data) and prevent data anomalies (errors when inserting, updating, or deleting).
...
Clustered Index: Dictates the actual physical sorting order of the data on the disk. Because data can only be physically sorted one way, you can only have one clustered index per table (usually the Primary Key).
...
OLTP (Online Transaction Processing): Designed for fast, short, real-time inserts/updates (e.g., an e-commerce checkout, ATMs). Highly normalized.

OLAP (Online Analytical Processing): Designed for complex queries, data mining, and reading massive amounts of historical data (e.g., calculating year-over-year corporate sales). Uses denormalized data warehouses.
